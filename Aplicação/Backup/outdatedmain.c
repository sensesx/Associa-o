...
int commandParser(char *command, char *pwdDir, int pwdLen);
int info();
				...
				case 1:
					if(atoi(option) == 1){
						...
						while(true){ 
							...
							if(commandParser(userCommand, mainpwd, pwdLen) == EXIT){
								break;
							}
							free(userCommand); 
						}
...
int commandParser(char *command, char *pwdDir, int pwdLen){
	...
	if(strcmp(command, "1") == 0){
		...
		if((lineDigitDel = delMember())){

			delFpFile = fopen(pwdDir, "r");
			tempDel = fopen("temp.txt", "w");
			while(fgets(lines, sizeof(lines), delFpFile)){
				delCountFunc++;
				if(delCountFunc == lineDigitDel){
					printf("igual");
					continue;
				}
				fputs(lines, tempDel);
...
char commandBuffer[2048], commandPtrLen;
char *commandPtr = NULL; 
int commandBufferLen;
	
	USER = getenv("USER");
	printf("\033[1;31m[Painel Associação] \033[0;32m- \033[1;35m%s: \033[1;0m%s ", USER, prompt);
	while(fgets(commandBuffer, sizeof(commandBuffer), stdin)){
		commandBufferLen = strlen(commandBuffer);
		if(commandPtr == NULL){
			commandPtr = (char *) malloc(commandBufferLen + 1);

		}else{
			char *realloCommandPtr = realloc(commandPtr, commandPtrLen+commandBufferLen+1);
			if(realloCommandPtr != NULL){
				commandPtr = realloCommandPtr;
			}else{
				free(commandPtr);
				commandPtr = NULL;
			}
		...
		if(commandPtr == NULL){
			printf("%s Erro: %s", n, strerror(errno));
			return NULL;
		}
		if(!strcpy(commandPtr+commandPtrLen, commandBuffer)){
			printf("%s Erro: %s", n, strerror(errno));
		}
		if(commandBuffer[commandBufferLen-1] == '\n')
		{
		    if(commandBufferLen == 1 || commandBuffer[commandBufferLen - 2] !='\\'){
			return commandPtr;
		    }

		    commandPtr[commandPtrLen+commandBufferLen-2] = '\0';
		    commandBufferLen -= 2;
		}
		commandPtrLen += commandBufferLen;
	}
